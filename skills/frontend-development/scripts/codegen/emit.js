...
function contracts(page, apiBase) {
  ...
  for (const obj of page.objectTypes || []) {
    ...
    out.push(
      `/** \`${obj.label}\` — a nested object on the wire, never a string. */\n` +
  ...
  for (const cmd of page.commands) {
    ...
    out.push(
      `/** Body of \`POST ${apiBase}${cmdPath(cmd)}\` — the \`${cmd.id}\` command. */\n` +
        `export interface ${naming.payload(cmd.id)} {\n${fields}\n}\n\n` +
        `export const ${constName(cmd.id)}_ENDPOINT = '${apiBase}${cmdPath(cmd)}';`,
    );
  }
  for (const view of page.views) {
    const fields = view.fields.length
      ? view.fields.map((f) => `  ${f.name}: ${f.tsType};`).join('\n')
      : '  // this read model projects no columns';
    out.push(
...
// The URL path an element is served at. Comes from openapi.json when the backend
// publishes one, so a literal can never drift from the controller; the model id
// is only the fallback for a project without a published contract.
const cmdPath = (cmd) => cmd.endpoint || `/${cmd.id}`;
const viewPath = (view) =>
  view.endpoint || (view.collection ? `/${view.id}` : `/${view.id}/{aggregateId}`);
...
function initValue(f, indent) {
  ...
  // A list starts with one empty entry so the form renders one input for it.
  if (f.tsType.endsWith('[]')) return `[${initValue({ ...f, tsType: f.tsType.slice(0, -2) }, indent)}]`;
  if (f.tsType === 'number') return '0';
